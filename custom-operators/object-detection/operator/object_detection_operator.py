...
async def init(memo: kopf.Memo, **kwargs) -> None:
    ...
    """
    app_type = "TYPE_OBJECT_DETECTION"
    default_chart_name = "object-detection-helm"
    ...
    memo.bookkeeping_path = memo.mounted_path + "object_detection_bookkeeping.yml"
    ...
    memo.versioning_path = memo.mounted_path + "object_detection_versioning.yml"
    memo.versioning = await load_yaml_as_dict(memo.versioning_path)  


@kopf.on.create("ika.rwth-aachen.de", "v1", "objectdetections")
async def create_object_detection(memo, spec, body, **kwargs):
    """Handles the creation of a new Custom Resource 'objectdetections'.
    ...
    logging.info(f"[Object Detection Operator] Custom Resource '{cr_name}' was created.")
    config = dict(spec) # shallow copy, as some keys will be popped
    config_operator = config.pop("operator", None)
    verify_operator_config(config_operator, "kopf.on.create('objectdetections')")
    assert not config_operator["shutdown"], "[Object Detection Operator] kopf.on.create('objectdetections'): Shutdown is requested but CR is not deployed yet."
    ...
    logging.info(f"[Object Detection Operator] Requesters of CR '{cr_name}' are: {config_operator['requester']}")
    ...
    logging.info(f"[Object Detection Operator] Helm release for CR '{cr_name}' installed with release name '{cr_name}'")
...
@kopf.on.update("ika.rwth-aachen.de", "v1", "objectdetections")
async def update_object_detection(memo, spec, body, retry, **kwargs):
    """Handles the update of an existing Custom Resource 'objectdetections'.
    ...
    """
    cr_name = body['metadata']['name']
    logging.info(f"[Object Detection Operator] Custom Resource '{cr_name}' was updated.")
    config = dict(spec) # shallow copy, as some keys will be popped
    config_operator = config.pop("operator", None)
    verify_operator_config(config_operator, "kopf.on.update('objectdetections')")
    memo.versioning[cr_name][body['metadata']['resourceVersion']] = config_operator
    await save_dict_as_yaml(path=memo.versioning_path, dictionary=memo.versioning)    
    match config_operator["shutdown"]:
        case True:
            ...
            if not memo.bookkeeping[cr_name]:
                ...
                logging.info(f"[Object Detection Operator] No Requesters left. Delete CR '{cr_name}' ..")
    ...
    logging.info(f"[Object Detection Operator] According to Bookkeeping, Requesters of CR '{cr_name}' are: '{memo.bookkeeping[cr_name]}'")


@kopf.on.delete("ika.rwth-aachen.de", "v1", "objectdetections")
async def delete_object_detection(memo, spec, body, **kwargs):
    """Handles the deletion of an existing Custom Resource 'objectdetections'.
    ...
    """
    cr_name = body['metadata']['name']
    logging.info(f"[Object Detection Operator] Custom Resource '{cr_name}' was deleted.")
...
def verify_operator_config(config: dict, caller_name: str) -> None:
    ...
    assert config.get("requester", None) is not None, (
        caller_name + " cannot find the requester of the object detection in the operator config"
    )
